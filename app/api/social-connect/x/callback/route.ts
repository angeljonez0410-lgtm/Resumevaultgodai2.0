...
export async function GET(req: NextRequest) {
  try {
    ...
    const redirectUri = new URL("/api/social-connect/x/callback", req.url).toString();
    ...
    if (testTweet === "1") {
      await createXTweet(tokenData.access_token, "Social Bot is now connected to X. Ready to publish content.");
    }
