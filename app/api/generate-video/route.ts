...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const enhancedPrompt = [
      typeof characterDescription === "string" && characterDescription.trim()
        ? `${characterDescription.trim()}.`
        : "",
      `${prompt.trim()}. Realistic cinematic short video, natural movement, stable camera, professional lighting, no warping, no extra limbs, no text artifacts.`,
    ]
      .filter(Boolean)
      .join(" ");
